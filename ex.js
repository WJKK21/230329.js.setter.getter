...
* return {boolean}
*
* 이메일 형식 @ 표시가 있는지 여부와 등록된 이메일 주소인지를 검사해주는 함수
*가정
* let emailAddress= ["gmail.com", "naver.com", "daum.net"];
*/
    ...
    Array.from(email).forEach((element) => {
      ...
      console.log(emailSplit);
        agreeEmailAddress.filter((element) => {
          if(element ===
          emailSplit[1]) { // 매개변수로 받은 이메일 주소 목록(가정)과 일치하는지 검사 console.log("이메일 도메인이 일치합니다.");
